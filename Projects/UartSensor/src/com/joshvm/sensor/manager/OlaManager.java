...
public class OlaManager extends SensorManager implements SensorCallback {
	...
	public OlaManager(SensorInterface sensorInterface) {
	...
	public void sendHand() {
		...
		uartStream.send(Constant.SENSOR_API_OLA);
	}
	...
	public void sendAuto(int rate) {
		...
		uartStream.send(Constant.SENSOR_API_AUTO_OLA);
				...
				if (b[i] == Utils.sumByteArray(dataBuffer, 0, dataPoi)) {
					...
					// 欧拉角计算,加速度计算，陀螺仪计算。
					 if (dataBuffer[2] == (byte) 0x45) {
						...
						sensorInterface.onReceive(Constant.SENSOR_OLA, x, y, z);
					} 
