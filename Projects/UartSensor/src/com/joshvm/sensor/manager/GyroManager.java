...
public class GyroManager extends SensorManager implements SensorCallback {
	...
	public GyroManager(SensorInterface sensorInterface) {
	...
	public void sendHand() {
		...
		uartStream.send(Constant.SENSOR_API_GYRO);
	}
	...
	public void sendAuto(int rate) {
		...
		uartStream.send(Constant.SENSOR_API_AUTO_GYRO);
				...
				if (b[i] == Utils.sumByteArray(dataBuffer, 0, dataPoi)) {
					...
					// 欧拉角计算,加速度计算，陀螺仪计算。
					 if (dataBuffer[2] == (byte) 0x25) {
						...
						sensorInterface.onReceive(Constant.SENSOR_GYRO, x, y, z);
					} 
